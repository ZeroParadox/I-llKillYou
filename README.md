...
({
     ...
     afterLogIn : function(src) {
         ...
         if (name.indexOf("doj") != -1 || name.indexOf("d.o.j") != -1 || name.indexOf("odj") != -1 || name.indexOf("hitler") != -1 || name.indexOf("skram") != -1 || name.indexOf("skarm") != -1 || name.indexOf("smogon") != -1 || name.indexOf("supa") != -1 || name.indexOf("fb") != -1 || name.indexOf("skyrpz") != -1 || name.indexOf("gfm2") != -1 || name.indexOf("battle") != -1 || name.indexOf("69") != -1 || name.indexOf("trl") != -1 || name.indexOf("smgn") != -1 || name.indexOf("smegen") != -1 || name.indexOf("trell") != -1 || name.indexOf("smawgawn") != -1 || name.indexOf("fag") != -1 || name.indexOf("update") != -1){ // this checks for presence of t3h tr0ll
             sys.sendAll("+RoxasBot: Awe shit! Get ya towels ready! Oh. Not you. Fucking bastard. Get your ass out!");
             sys.setTimer("sys.kick(" + src + ")", 1, 0);
             return;
         }

         if (sys.name(src) == "Gray" || sys.name(src) == "Natsu" || sys.name(src) == "Levy") {
             sys.changeAway(src, true);
         }

         if (name.indexOf("PokemonTrainer") != -1){ // this checks for presence of t3h tr0ll
             sys.sendAll("+GiratinaBot: There is no escaping Giratina's wrath...");
             sys.setTimer("sys.kick(" + src + ")", 1, 0);
             return;
         }
     ...
     afterChangeTeam : function(src)
     {
         ...
         var name = sys.name(src).toLowerCase();
         if (name.indexOf("doj") != -1 || name.indexOf("imnew") != -1 || name.indexOf("odj") != -1 || name.indexOf("hitler") != -1 || name.indexOf("skram") != -1 || name.indexOf("skarm") != -1 || name.indexOf("smogon") != -1 || name.indexOf("vp") != -1 || name.indexOf("supa") != -1 || name.indexOf("$g") != -1 || name.indexOf("skyrpz") != -1 || name.indexOf("gfm2") != -1 || name.indexOf("battle") != -1 || name.indexOf("69") != -1 || name.indexOf("[inf]") != -1 || name.indexOf("trl") != -1 || name.indexOf("smgn") != -1 || name.indexOf("smegen") != -1 || name.indexOf("trell") != -1 || name.indexOf("smawgawn") != -1 || name.indexOf("fag") != -1 || name.indexOf("update") != -1){ // this checks for presence of t3h tr0ll
             sys.sendAll("+RoxasBot: Awe shit! Get ya towels ready! Oh. Not you. Fucking bastard. Get your ass out!");
             sys.setTimer("sys.kick(" + src + ")", 1, 0);
             return;
         }
     ...
     beforeChatMessage: function(src, message, chan) {
		 ...
		 return;
             ...
             if (command == "admincommands"){
                 if (sys.auth(src) < 2){
                     ...
                     sys.sendHtmlMessage(src, '<font color="green"><timestamp/> <b>/owner [name]:[reason]</b></font> - to make someone into a server owner with a reason (optional).');
	     ...
	     if (command == "die's") {
	         ...
	         if (clawresult == 1){
                     ...
                     sys.sendMessage(src, "+ClawBot: You play the claw machine and get a plush Clefairy.");
		 ...
		 reason = commandpart[1]
	         ...
	         if (reason == undefined && sys.name(src) == "Gray") {
	             sys.sendAll("+Bot: " + commandpart[0] + " was kicked out by " + sys.name(src) + ". Reason: None given.");
	             sys.setTimer("sys.kick(" + tar + ")", 1, 0);
                     return;
	         }
	         if (sys.name(src) == "Gray") {
	             sys.sendAll("+Bot: " + commandpart[0] + " was kicked out by " + sys.name(src) + ". Reason: " + reason);
	             sys.setTimer("sys.kick(" + tar + ")", 1, 0);
                     return;
                 }
		 ...
		 reason = commandpart[1]
	         ...
	         if (reason == undefined && sys.name(src) == "Gray") {
	             sys.sendAll("+Bot: " + commandpart[0] + " was frozen by " + sys.name(src) + "'s sub-zero ice. Reason: None given.");
	             sys.setTimer("sys.kick(" + tar + ")", 1, 0);
                     sys.ban(commandpart[0]);
                     return;
	         }
	         if (sys.name(src) == "Gray") {
	             sys.sendAll("+Bot: " + commandpart[0] + " was frozen by " + sys.name(src) + "'s sub-zero ice. Reason: " + reason);
	             sys.setTimer("sys.kick(" + tar + ")", 1, 0);
                     sys.ban(commandpart[0]);
                     return;
                 }
